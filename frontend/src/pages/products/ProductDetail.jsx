...
import { useParams, Link, useNavigate } from "react-router-dom"
import { IoIosArrowRoundBack, IoMdDownload } from "react-icons/io"
import { FaQrcode, FaCoins, FaTag, FaCalendar, FaCheckCircle, FaClock } from "react-icons/fa"
import { Loader2, AlertTriangle, Download, ChevronDown, ChevronUp } from "lucide-react"
import toast from "react-hot-toast"
...
export default function ProductDetail() {
  ...
  const downloadSelectedQRs = () => {
    const selected = filteredQRCodes.filter((qr) => selectedQRs.includes(qr._id))
  ...
  const downloadByDateRange = () => {
    ...
    downloadBulkQRCodes(dateFilteredQRs, `${product.productId}_${dateFrom}_to_${dateTo}_qrcodes`)
    setDateFrom("")
    setDateTo("")
  }
  ...
  const activeQRCodes = product.qrCodes.filter((qr) => qr.qrStatus === "active")
  const scannedQRCodes = product.qrCodes.filter((qr) => qr.qrStatus === "scanned")

  const filteredQRCodes =
    qrStatus === "active" ? activeQRCodes : qrStatus === "scanned" ? scannedQRCodes : product.qrCodes
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#C3E8FF] to-white px-4 sm:px-6 lg:px-10 pt-7 pb-12">
      ...
      {/* Back Button */}
      <div className="mb-6">
        <Link to="/products" className="inline-flex items-center text-gray-700 hover:text-black font-medium transition">
          <IoIosArrowRoundBack size={32} className="mr-1" />
          Back to Products
        </Link>
      </div>
        ...
        <div className="md:flex">
          ...
          <div className="md:w-2/3 p-8">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-2 bg-yellow-50 px-4 py-2 rounded-full border border-yellow-200">
                <FaCoins className="text-yellow-600" />
                <span className="text-xl font-bold text-yellow-700">{product.coinReward}</span>
                <span className="text-sm text-yellow-600">coins</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                ...
                <p className="text-lg font-semibold text-blue-900">{product.category?.categoryName || "N/A"}</p>
              </div>

              <div className="bg-green-50 rounded-lg p-4 border border-green-100">
                <div className="flex items-center gap-2 mb-1">
                  <FaCheckCircle className="text-green-600" />
                  <span className="text-xs text-green-600 font-medium">Active QR</span>
                </div>
                <p className="text-lg font-semibold text-green-900">{activeQRCodes.length}</p>
              </div>

              <div className="bg-purple-50 rounded-lg p-4 border border-purple-100">
                <div className="flex items-center gap-2 mb-1">
                  <FaClock className="text-purple-600" />
                  <span className="text-xs text-purple-600 font-medium">Scanned</span>
                </div>
                <p className="text-lg font-semibold text-purple-900">{scannedQRCodes.length}</p>
              </div>

              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-center gap-2 mb-1">
                  <FaQrcode className="text-gray-600" />
                  <span className="text-xs text-gray-600 font-medium">Total QR</span>
                </div>
                <p className="text-lg font-semibold text-gray-900">{product.qrCodes.length}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600 mb-1">Product ID</p>
                    <p className="font-mono font-semibold text-gray-900">{product.productId}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600 mb-1">Product Name</p>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600 mb-1">Category</p>
                    <p className="font-semibold text-gray-900">{product.category?.categoryName || "N/A"}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600 mb-1">Coin Reward</p>
                    <p className="font-semibold text-yellow-700 flex items-center gap-2">
              ...
              <div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6 border border-green-200">
                    <p className="text-sm text-green-700 mb-2">Active QR Codes</p>
                    <p className="text-3xl font-bold text-green-900">{activeQRCodes.length}</p>
                    <p className="text-xs text-green-600 mt-1">Ready to scan</p>
                  </div>
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6 border border-blue-200">
                    <p className="text-sm text-blue-700 mb-2">Scanned QR Codes</p>
                    <p className="text-3xl font-bold text-blue-900">{scannedQRCodes.length}</p>
                    <p className="text-xs text-blue-600 mt-1">Already used</p>
                  </div>
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-6 border border-purple-200">
                    <p className="text-sm text-purple-700 mb-2">Total QR Codes</p>
                    <p className="text-3xl font-bold text-purple-900">{product.qrCodes.length}</p>
                    <p className="text-xs text-purple-600 mt-1">All codes</p>
                  </div>
                </div>
              </div>
            ...
            <div>
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  <h3 className="text-lg font-semibold text-gray-900">All QR Codes</h3>
                  <select
                    id="qrStatus"
                    name="qrStatus"
                  ...
                  >
                    ...
                    <option value="active">Active ({activeQRCodes.length})</option>
                    <option value="scanned">Scanned ({scannedQRCodes.length})</option>
                  </select>
                </div>

                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => downloadBulkQRCodes(filteredQRCodes, `${product.productId}_${qrStatus}_qrcodes`)}
                  ...
                  >
                    {isBulkDownloading ? (
                      ...
                      <>
                        <Download size={18} />
                        Download {qrStatus === "all" ? "All" : qrStatus.charAt(0).toUpperCase() + qrStatus.slice(1)} (
              ...
              <div className="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200 overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-gray-900">Advanced Download Options</span>
                  ...
                  <div className="p-4 border-t border-blue-200 bg-white space-y-4">
                    ...
                    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <button
                        ...
                        disabled={selectedQRs.length === 0 || isBulkDownloading}
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3 p-3 bg-gray-50 rounded-lg">
                      <div className="flex-1 w-full">
                        <label className="block text-sm font-medium text-gray-700 mb-1">Download Top N QR Codes</label>
                        <input
                          type="number"
                          value={topNCount}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                        />
                        <span className="text-xs text-gray-600 font-medium">Select</span>
                      </label>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <div>
                          ...
                          <p className="font-mono text-sm font-semibold text-gray-900 break-all">{qr.qrCode}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 mb-1">Created At</p>
                          <p className="text-sm font-semibold text-gray-900">
                            {qr.createdAt ? formatDate(qr.createdAt) : "not provided"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          ></span>
                          {qr.qrStatus?.charAt(0).toUpperCase() + qr.qrStatus?.slice(1)}
                        </span>
                      </div>
                      ...
                      {qr.scannedBy && (
                        <div>
                          ...
                          <p className="text-sm text-gray-900">{qr.scannedBy}</p>
                        </div>
                      )}
                      ...
                      {qr.scannedAt && (
                        <div>
                          ...
                          <p className="text-sm text-gray-900">{formatDate(qr.scannedAt)}</p>
                        </div>
                      )}

                      {/* Download Button */}
                      <button
                        onClick={() => downloadQRCode(qr.qrCodeImage, qr.qrCode)}
                        className="w-full mt-3 flex items-center justify-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition text-sm font-medium"
                      >
                        <IoMdDownload size={18} />
                        Download
              ...
              {filteredQRCodes.length === 0 && (
                <div className="text-center py-12">
                  <FaQrcode className="mx-auto h-12 w-12 text-gray-400 mb-4" />
                  <p className="text-gray-600">
                    No {qrStatus !== "all" ? qrStatus : ""} QR codes available for this product.
                  </p>
                </div>
              )}
            </div>
